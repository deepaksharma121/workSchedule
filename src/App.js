...
function App(){
  ...
  const addTask = () =>{
    if(task.trim() === "" || deadline === ""){
      alert("Task and Deadline are required");
      return;
    }
  ...
  return (
    <div className='App'>
      <header>
        <h1>Everyday Working Schedule</h1>
      </header>
      <main>
        <div className='task-form'>
          <input type='text' id='task' placeholder='Enter your task' value={task} onChange={handleTaskChange} />
            ...
            <tbody>
              {upcomingTasks.map((t) => (
                <tr key={t.id}>
                  ...
                  <td>
                    {!t.done && (
                      <button className='mark-done' onClick={() => markDone(t.id)}>Mark Done</button>
                    )}
                  </td>
                </tr>
              ))}
      ...
      <footer>
        <p> © Everyday Working Schedule. Developed by Deepak Sharma.</p>
      </footer>
    </div>
